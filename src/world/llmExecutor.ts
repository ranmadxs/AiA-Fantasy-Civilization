...
function buildPrompt(context: NationTurnContext, config: NationModelConfig): string {
  ...
  return [
    ...
    `  "upgrade":{"provinceId":"...","kind":"stable"}: upgrade a stable (lvl1→2 needs lvl2 city + 3 tiles; lvl2→3 needs lvl3 city + 5 tiles, unlocks carts). "buildOrders":[{"provinceId":"...","kind":"granja"}]: explicit constructions, validated (own province, era-unlocked, funds). provinceId MUST be a province id (a city id is also accepted and resolved to its province). Invalid ones are rejected with a reason; the chain fills remaining slots.`,
    ...
    ``,
     ...
     `  "muster":[{"cityId":"...","units":{"militia":30},"stance":"attack"}]: form an army group from that city's garrison (cityId MUST be a city id; a province id is accepted and uses its largest garrison; amounts within movable, total >= 25, stance optional attack/defend/garrison/rally/raid/retreat). Omit or null for automated mustering.`,
